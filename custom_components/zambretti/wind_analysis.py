...
async def calculate_most_frequent_wind_direction(hass, entity_id):
    ...
    try:
        if len(history_data) > 0:
            _LOGGER.debug(f"✅ Found history for {entity_id}, total records: {len(history_data[entity_id])}")
        else:
    ...
    if not wind_directions:
        ...
        return "Error: Wind direction not available.", 0  # If even the current state is missing
    
    # Find the most frequent wind direction
    return collections.Counter(wind_directions).most_common(1)[0][0], len(history_data[entity_id])
...
async def determine_wind_speed(hass, entity_id):
    ...
    if not history_data or entity_id not in history_data:
        ...
        current_state = hass.states.get(entity_id)
        if current_state:
            return safe_float(current_state.state), 1  # Returning 1 as a single data point
    ...
    _LOGGER.debug(f"✅ Calculated average wind speed: {average_wind_speed} knots over {len(wind_speed_values)} records.")
...
def determine_wind_direction(wind_direction, pressure_trend):
    ...
    _LOGGER.debug(f"🔄 Estimating wind direction shift from {wind_direction} with trend: {pressure_trend}")

    # Define the wind direction order in a 16-point compass rose
    compass_directions = ["N", "N-NE", "NE", "E-NE", "E", "E-SE", "SE", "S-SE",
                          "S", "S-SW", "SW", "W-SW", "W", "W-NW", "NW", "N-NW"]

    # **Mapping for cardinal direction conversion**
    veering_map = {  # Moving clockwise
        "N": "E", "N-NE": "E", "NE": "E", "E-NE": "E",
        "E": "S", "E-SE": "S", "SE": "S", "S-SE": "S",
        "S": "W", "S-SW": "W", "SW": "W", "W-SW": "W",
        "W": "N", "W-NW": "N", "NW": "N", "N-NW": "N"
    }

    backing_map = {  # Moving counterclockwise
        "N": "W", "N-NE": "N", "NE": "N", "E-NE": "N",
        "E": "N", "E-SE": "E", "SE": "E", "S-SE": "E",
        "S": "E", "S-SW": "S", "SW": "S", "W-SW": "S",
        "W": "S", "W-NW": "W", "NW": "W", "N-NW": "W"
    }
