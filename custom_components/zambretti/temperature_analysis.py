...
async def determine_temperature_effect(hass, entity_id):
    ...
    if sun_state and "next_rising" in sun_state.attributes and "next_setting" in sun_state.attributes:
        sunrise = dt_util.parse_datetime(sun_state.attributes["next_rising"]).astimezone(dt_util.DEFAULT_TIME_ZONE)
        sunset = dt_util.parse_datetime(sun_state.attributes["next_setting"]).astimezone(dt_util.DEFAULT_TIME_ZONE)
    else:
        _LOGGER.debug("⚠️ Could not retrieve sunrise/sunset times from sun.sun. Skipping adjustments.")
    ...
    in_sunrise_window = sunrise_window_start.time() <= current_time <= sunrise_window_end.time()
    in_sunset_window = sunset_window_start.time() <= current_time <= sunset_window_end.time()

    _LOGGER.debug(f"🌅 Sunrise: {sunrise}, Sunset: {sunset}")
    _LOGGER.debug(f"🌅 Current Time: {current_time} | Sunrise Window: {in_sunrise_window} | Sunset Window: {in_sunset_window}")
    ...
    alert_level = 0
    if temp_change >= 10:
        temp_effect = "Rapid significant warming; potential heatwave, strong thermal winds"
        alert_level = 3
    elif temp_change >= 5:
        temp_effect = "Noticeable temperature rise ; warm air front moving in, wind increase"
    elif temp_change <= -10:
        temp_effect = "Sharp temperature drop; cold front, strong gusty winds and storms"
        alert_level = 5
    elif temp_change <= -5:
